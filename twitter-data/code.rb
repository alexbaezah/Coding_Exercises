...
#YOUR CODE GOES HERE
def Description()
  twitter_data.each do |info|
    info.each do |name,infos|
      puts "#{name}: #{infos["description"]}"
    end
  end
end
...
def Total_Follower()
  twitter_data.each do |info|
    info.each do |name, infos|
      puts "#{name}: #{infos["number of followers"]}"
    end
  end
end
...
def Latest_Tweet_Length()
  twitter_data.each do |info|
    info.each do |name, infos|
      puts "#{name}'s latest tweet was #{infos["latest tweet"].length} characters long."
    end
  end
end


def Total_Char_Last20tweet()
  count = 0

  twitter_data.each do |info|
    info.each do |name, infos|
      infos["last twenty tweets"].each do |tweet|
        count += tweet.length
      end
      puts "#{name} used #{count} characters in their last twenty tweets."
    end
  end
end
